...
import com.ve.cesarrosales.entitysystem.component.Component;
	...
	public <T extends Component> T getComponent(UUID entity,
		...
		synchronized (componentStores) {
			...
			if (result == null) {
				 ...
				 * 
				 * 
				 * StringBuffer sb = new StringBuffer(); for( UUID e :
				 * store.keySet() ) { sb.append(
				 * "\nUUID: "+e+" === "+store.get(e) ); }
				 * 
				 * throw new IllegalArgumentException("GET FAIL: " + entity +
				 * "(name:"+nameFor(entity)+")" +
				 * " does not possess Component of class\n   missing: " +
				 * componentType + "TOTAL STORE FOR THIS COMPONENT CLASS : "+
				 * sb.toString() );
				 */
			}
